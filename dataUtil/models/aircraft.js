...
AircraftSchema.methods.validateAircraft = async function () {
  ...
  if (!mongoose.Types.ObjectId.isValid(this.team))
    nexusError("Invalid Team ID given...", 400);
  const team = await Team.findById(this.team);
  if (team.length < 1)
    nexusError(`No team exists with the ID: ${this.team}`, 400);

  if (this.origin === undefined)
    nexusError("No facility ID given for origin...", 400);
  if (!mongoose.Types.ObjectId.isValid(this.origin))
    nexusError("Invalid facility ID given for origin...", 400);
  const facility = await Facility.findById(this.origin);
  if (facility.length < 1)
    nexusError(`No facility exists with the ID: ${this.origin}`, 400);

  if (this.site === undefined) nexusError("No site ID given...", 400);
  if (!mongoose.Types.ObjectId.isValid(this.site))
    nexusError("Invalid site ID given...", 400);
  const site = await Site.findById(this.site);
  if (site.length < 1)
    nexusError(`No site exists with the ID: ${this.site}`, 400);

  if (this.zone === undefined) nexusError("No zone ID given...", 400);
  if (!mongoose.Types.ObjectId.isValid(this.zone))
    nexusError("Invalid zone ID given...", 400);
  const zone = await Zone.findById(this.zone);
  if (zone.length < 1)
    nexusError(`No zone exists with the ID: ${this.zone}`, 400);

  if (this.country === undefined) nexusError("No country ID given...", 400);
  if (!mongoose.Types.ObjectId.isValid(this.country))
    nexusError("Invalid country ID given...", 400);
  const country = await country.findById(this.country);
  if (country.length < 1)
    nexusError(`No country exists with the ID: ${this.country}`, 400);
};
...
AircraftSchema.methods.launch = async (mission) => {
  ...
  try {
    ...
    await banking.withdrawal(
      account,
      1,
      `Mission funding for ${mission.toLowerCase()} flown by ${this.name}`
    );
...
AircraftSchema.methods.recall = async () => {
  ...
  try {
    const { origin } = this;
    const home = await Facility.findById(origin).populate("site");
