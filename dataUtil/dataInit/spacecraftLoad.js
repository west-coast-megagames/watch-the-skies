...
async function initLoad(doLoad) {
  ...
  for (let data of spacecraftDataIn) {
    ++recReadCount;
  ...
  try {
    ...
    if (!spacecraft) {
        ...
        for (let fac of iData.facilities) {
          ...
          if (facRef) {
            if (validUnitType(facRef.unitType, "Spacecraft")) {
              ...
              if (facType == "Factory") {
                ...
                logger.error(
                  `Invalid Facility Type In spacecraft Load: ${facRef.type}`
                );
                facError = true;
              }
            } else {
              logger.debug(
                `Error in creation of facility ${fac} for  ${spacraft.name} - wrong unitType`
              );
              facError = true;
            }
          } else {
            logger.debug(
              `Error in creation of facility ${fac} for  ${spacecraft.name}`
            );
            facError = true;
          }
          ...
          if (!facError) {
            ...
            try {
              ...
              logger.error(
                `New Spacecraft Facility Save Error: ${err.message}`,
                { meta: err }
              );
      ...
      if (loadError) {
        ...
        try {
          let spacecraftSave = await spacecraft.save();
          ++rCounts.loadCount;
          spacecraftDebugger(
          ...
          if (spacecraftSave.shipType === "Satellite") {
            addSatelliteToZone(
              spacecraftSave._id,
              spacecraftSave.zone,
              spacecraftSave.team
        ...
        for (let fac of iData.facilities) {
          ...
          if (facRef) {
            if (validUnitType(facRef.unitType, "Spacecraft")) {
              ...
              if (facType == "Factory") {
                ...
                logger.error(
                  "Invalid Facility Type In Spacecraft Load:",
                  facRef.type
                );
                facError = true;
              }
              ...
              logger.debug(
                `Error in creation of facility ${fac} for  ${spacecraft.name} - wrong unitType`
              );
              facError = true;
            }
          } else {
          ...
          if (!facError) {
            ...
            try {
              ...
              delFacilities(spacecraft.facilities);
              logger.error(
                `Update Spacecraft Facility Save Error: ${err.message}`,
                { meta: err }
              );
