...
const { Aircraft } = require("../../models/aircraft"); // Mongoose Model - Aircraft
...
async function withdrawal(account, amount, note) {
  ...
  bankDebugging(
    `${amount} witdrawn from ${account.owner}'s ${account.name} account.`
  );
  bankDebugging(`Reason: ${note}`);
...
async function setAutoTransfer(to, from, amount, note) {
  ...
  await account.save();
  console.log(
    `${account.owner} has set up an auto-transfer for ${account.name}`
  );
      ...
      for (let transfer of account.autoTransfers) {
        if (transfer !== null) {
          ...
          if (amount < withdrawalAccount.balance) {
            ...
            withdrawalAccount = await withdrawal(
              withdrawalAccount,
              amount,
              note
            );
...
function trackTransaction(account, amount, type) {
  ...
  if (type === "deposit") {
    ...
    bankDebugging(
      `Deposit of ${amount} tracked on the ${account.name} account...`
    );
    account.markModified("deposits");
  } else if (type === "withdrawal") {
    account.withdrawals[turnNum] += amount;
    bankDebugging(
      `Withdrawal of ${amount} tracked on the ${account.name} account...`
    );
