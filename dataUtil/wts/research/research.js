...
async function startResearch() {
  for await (const lab of await Facility.find({ type: "Lab" })) {
...
async function calculateProgress(lab) {
  ...
  for await (const project of lab.research) {
    ...
    try {
      ...
      if (tech != null) {
        ...
        const sciBonus = lab.bonus;
        researchDebugger(`Science Rate: ${sciRate}`);
        const progressInfo = await researchMultiplyer(
          sciRate,
          lab.funding,
          sciBonus
...
function researchMultiplyer(sciRate, funding, sciBonus) {
  ...
  const progress = sciRate * multiplyer; // Final calculation of progress on the technology
...
async function completeTech(research, lab) {
  ...
  for await (const item of research.unlocks) {
    ...
    if (research.type === "Knowledge") {
      if (research.level < 5) {
        const nextKnowledge = knowledgeTree.find(
