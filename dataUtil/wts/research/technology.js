...
function Technology(tech) {
  ...
  this.checkAvailable = async function () {
    for await (let team of await Team.find({ type: "N" }, "_id name")) {
      let currentTech = await Research.findOne({
        name: this.name,
        team: team._id,
      }); // Checks if team has the research already!!
      techDebugger(
        `${this.name}: Checking ${team.name}'s eligibility to research this tech...`
      );
      if (currentTech === null) {
        let count = 0;
        let unlock = false;
        ...
        for await (let req of this.prereq) {
          ...
          let checkTech = await Research.findOne({
            code: req.code,
            team: team._id,
          ...
          if (checkKnowledge !== null) {
            if (checkKnowledge.status.completed) {
              if (
                checkKnowledge.credit.toHexString() === team._id.toHexString()
              ) {
                techDebugger(
                  `COMPLETED - ${this.name} is available to research for ${team.name}...`
                );
              } else if (checkKnowledge.status.published) {
                techDebugger(
                  `PUBLISHED - ${checkKnowledge.name} is public information...`
                );
              } else {
                techDebugger(
                  `UNKNOWN - ${checkKnowledge.name} is not available to ${team.name}...`
                );
                checkKnowledge = null;
              }
            }
        ...
        if (unlock === true) {
          ...
          for await (let unlock of newTech.unlocks) {
            if (unlock.type === "Technology") {
              ...
              techDebugger(
                `It's gunna be the future soon: ${theory.type} - ${theory.name}`
              );
              console.log(theory);
  ...
  for await (let research of Research.find().populate("team")) {
    if (research.status.visible && !research.status.available) {
      ...
      for await (let req of research.prereq) {
        ...
        let checkTech = await Research.findOne({
          code: req.code,
          team: research.team._id,
          "status.completed": true,
        });
        let checkKnowledge = await Research.findOne({
          code: req.code,
        }).populate("credit");
        if (checkKnowledge !== null) {
          if (checkKnowledge.status.completed) {
            ...
            if (checkKnowledge.credit.name === research.team.name) {
              techDebugger(
                `COMPLETED - ${research.name} is available to research for ${research.team.name}...`
              );
            } else if (checkKnowledge.status.published) {
              techDebugger(
                `PUBLISHED - ${checkKnowledge.name} is public information...`
              ...
              techDebugger(
                `UNKNOWN - ${checkKnowledge.name} is not available to ${research.team.name}...`
              );
              checkKnowledge = null;
            }
          }
