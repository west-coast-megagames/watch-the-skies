...
async function loadGlobalVariables() {
  ...
  for (let team of await Team.find({ type: "N" })) {
...
async function knowledgeSeed() {
  ...
  for await (let field of fields) {
    ...
    let seed = await knowledgeTree.find(
      (el) => el.field === field && el.level === rand
    );
    seeded.push(seed);
  ...
  for await (let knowledge of seeded) {
    ...
    while (newKnowledge.level !== 0 && index !== -1) {
      index = tree.findIndex(
        (field) =>
          field.field === newKnowledge.field &&
          field.level === newKnowledge.level - 1
      );
      ...
      if (index != -1) {
        ...
        if (newKnowledge.teamProgress.length > 0) {
          rand = Math.floor(
            Math.random() * (newKnowledge.teamProgress.length - 1)
          );
  ...
  for await (let knowledge of await Research.find(
    { "status.completed": true },
    "name credit progress status"
  )) {
...
async function completeKnowledge(research) {
  ...
  knowledgeDebugger(
    `${team.name} has been credited with advancing the world to ${research.name}`
  );
