    ...
    if (stance === 'aggresive') {
        ...
        switch(stance === 'aggresive') {
            case (roll <= aggresiveArray[0]):
                interceptDebugger(`${name}'s outcome ${aggresiveArray[0]} or less - catastrophic failure.`)
            ...
            case (roll <= aggresiveArray[1]):
                interceptDebugger(`${name}'s outcome between ${aggresiveArray[0] + 1} and ${aggresiveArray[1]} - failure.`)
            ...
            case (roll <= aggresiveArray[2]):
                interceptDebugger(`${name}'s outcome between ${aggresiveArray[1] + 1} and ${aggresiveArray[2]} - Neutral Result.`);
                result.hit = true; // Neutral scores a hit on the target
                break;
            case (roll <= aggresiveArray[3]):
                interceptDebugger(`${name}'s outcome between ${aggresiveArray[2] +1 } and ${aggresiveArray[3]} - Mild Success.`);
    ...
    // Switch for if the UNIT stance is 'passive' | Recon / Transport / Diversion
        ...
        switch(stance === 'passive') {
            case (roll <= passiveArray[0]):
                interceptDebugger(`${name}'s outcome ${passiveArray[0]} or less` - `catastrophic failure.`)
                result.damage = 1;
                result.systemDmg = true;
                break;
            case (roll <= passiveArray[1]):
                interceptDebugger(`${name}'s outcome between ${passiveArray[0] + 1} and ${passiveArray[1]} - failure.`);
                result.damage = 1;
                result.systemDmg = false;
                break;
            case (roll <= passiveArray[2]):
                interceptDebugger(`${name}'s outcome between ${passiveArray[1] + 1} and ${passiveArray[2]} - neutral result.`);
                result.evade = unit.stats.evade + 1;
                break;
            case (roll <= passiveArray[3]):
                interceptDebugger(`${name}'s outcome between ${passiveArray[2] + 1} and ${passiveArray[3]} - mild success.`);
                result.evade = unit.stats.evade + 2;
                break;
            case (roll <= passiveArray[4]):
                interceptDebugger(`${name}'s outcome between ${passiveArray[3] + 1} and ${passiveArray[4]} - good success.`);
                result.evade = unit.stats.evade + 2;
                result.hit = true;
                break;
