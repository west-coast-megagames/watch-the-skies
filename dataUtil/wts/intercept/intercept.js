...
async function intercept(
  ...
  let interceptReport = await interceptDmg(
    attacker,
    defender,
    atkResult,
    defResult
  ); // Calculates damage and applies it | damage.js

  if (interceptReport.salvage.length > 0) {
    await generateCrash(
      interceptReport.salvage,
      attacker.site,
      attacker.country
    );
  }
  ...
  interceptDebugger(
    `Atk After Action Report - ${atkReport} ${interceptReport.atkReport}`
  );
  interceptDebugger(
    `Def After Action Report - ${defReport} ${interceptReport.defReport}`
  );
