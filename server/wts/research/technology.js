...
function Technology (tech) {
	...
	this.checkAvailable = async function () {
		for await (const team of await Team.find({ type: 'National' }, '_id name')) {
			...
			if (currentTech === null) {
				...
				if (unlock === true) {
					...
					techDebugger(msg);
					const newTech = new TechResearch({
						name: this.name,
						code: this.code,
						level: this.level,
						prereq: this.prereq,
	...
	this.unlock = async function (team) {
		const currentTech = await Research.findOne({ name: this.name, team: team }); // Checks if team has the research already!!
		if (currentTech === null) {
			...
			const theories = [];
			for await (const unlock of newTech.unlocks) {
				if (unlock.type === 'Technology') {
					const { techTree } = require('./techTree');
					const theory = techTree.find(el => el.code === unlock.code);
					techDebugger(`It's gunna be the future soon: ${theory.type} - ${theory.name}`);
					console.log(theory);
