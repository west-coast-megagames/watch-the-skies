...
const file = fs.readFileSync(config.get('initPath') + 'init-json/initArticle.json', 'utf8');
...
async function initLoad(doLoad) {
  ...
  for (let i = 0; i < articleDataIn.length; ++i ) {
    
    ++recReadCount;
    await loadArticle(articleDataIn[i], recCounts);
  }
  logger.info(`Article Load Counts Read: ${recReadCount} Errors: ${recCounts.loadErrCount} Saved: ${recCounts.loadCount} Updated: ${recCounts.updCount}`);
};
...
async function loadArticle(iData, rCounts){
  ...
  try {   
    ...
    await article.save((err, article) => {
      ...
      ++rCounts.loadCount;
      logger.info(`${article.headline} add saved to article collection`);
    });
...
async function deleteAllArticles(doLoad) {
  ...
  try {
    ...
    logger.error(`Delete All Articles Catch Error: ${err.message}`, {meta: err});
  }
};
