  ...
  for (const facility of await Facility.find()
    ...
    */
    ...
    if (!facility.hasOwnProperty('model')) {
      logger.error(`model missing for Facility ${facility.name} ${facility._id}`);
    }
    ...
    if (!facility.hasOwnProperty('team')) {
      logger.error(`team missing for Facility ${facility.name} ${facility._id}`);
    } else {
      let team = await Team.findById({'_id': facility.team});
      if (!team) {
        logger.error(`team reference is invalid for Facility ${facility.name} ${facility._id}`);
      }
    }

    if (!facility.hasOwnProperty('site')) {
      logger.error(`site missing for Facility ${facility.name} ${facility._id}`);
    } else {
      let site = await Site.findById({'_id': facility.site});
      if (!site) {
        logger.error(`site reference is invalid for Facility ${facility.name} ${facility._id}`);
      }
    }

    //check equipment references
    if (!facility.hasOwnProperty('equipment')) {
      logger.error(`equipment missing for Facility ${facility.name} ${facility._id}`);
    } else {
      ...
      for (let i = 0; i < facility.equipment.length; ++i){
        ...
        if (!eFind) {
          logger.error(`Facility ${facility.name} ${facility._id} has an invalid equipment reference ${i}: ${facility.equipment[i]}`);
        }
      }
    }

    if (!facility.hasOwnProperty('capacity')) {
      logger.error(`capacity missing for Facility ${facility.name} ${facility._id}`);
    }

    if (!facility.hasOwnProperty('status')) {  
      logger.error(`status missing for Facility ${facility.name} ${facility._id}`);
    } else {
      if (!facility.status.hasOwnProperty('repair')) {
        logger.error(`status.building missing for Facility ${facility.name} ${facility._id}`);
      }  
      if (!facility.status.hasOwnProperty('damaged')) {
        logger.error(`status.damaged missing for Facility ${facility.name} ${facility._id}`);
      }  
      if (!facility.status.hasOwnProperty('destroyed')) {
        logger.error(`status.destroyed missing for Facility ${facility.name} ${facility._id}`);
      }  
      if (!facility.status.hasOwnProperty('secret')) {
        logger.error(`status.secret missing for Facility ${facility.name} ${facility._id}`);
      }  
    }
    
    if (!facility.hasOwnProperty('hidden')) {  
      logger.error(`hidden missing for Facility ${facility.name} ${facility._id}`);
    }

    if (!facility.hasOwnProperty('type')) {
      logger.error(`type missing for Facility ${facility.name} ${facility._id}`);
    } else {
      
      if (!inArray(typeList, facility.type)) {
        logger.error(`Invalid type ${facility.type} for Facility ${facility.name} ${facility._id}`);
      }

      if (facility.type === 'Factory'){

        if (!facility.hasOwnProperty('project')) {
          logger.error(`project missing for Factory Facility ${facility.name} ${facility._id}`);
        } else {
      ...
      if (facility.type === 'Lab'){
        if (!facility.hasOwnProperty('research')) {  
          logger.error(`research missing for Lab Facility ${facility.name} ${facility._id}`);
        } else {
        ...
        if (!facility.hasOwnProperty('sciRate')) {  
          logger.error(`sciRate missing for Lab Facility ${facility.name} ${facility._id}`);
        } else {
        ...
        if (!facility.hasOwnProperty('bonus')) {  
          logger.error(`bonus missing for Lab Facility ${facility.name} ${facility._id}`);
        } else {
        ...
        if (!facility.hasOwnProperty('funding')) {  
          logger.error(`funding missing for Lab Facility ${facility.name} ${facility._id}`);
        } else {
    ...
    try {
      ...
      logger.error(`Facility Validation Error For ${facility.name} ${facility._id} Error: ${err.details[0].message}`);
    }
  }    
