...
async function chkAccount(runFlag) {
  ...
  const nameVals = ['Treasury', 'UNSC', 'Governance', 'Operations', 'Science', 'Political'];
  
  let teamCounts = [];
  //load teams
  for (const team of await Team.find()) {
    let tCnt = {hexId: team._id.toHexString(),
  ...
  for (const account of await Account.find()
    ...
    if (!account.hasOwnProperty('deposits')) {
      ...
      if (account.deposits.length < 15) {
        logger.error(`deposits has too few entries for Account ${account.name} ${account.owner} ${account._id}`);
      }    
    }
    ...
    if (!account.hasOwnProperty('withdrawals')) {
      ...
      if (account.withdrawals.length < 15) {
        logger.error(`withdrawals has too few entries for Account ${account.name} ${account.owner} ${account._id}`);
      }    
    }
    ...
    if (!account.hasOwnProperty('code')) {
      ...
      if (account.code === "" || account.code == undefined || account.code == null) {
        logger.error(`code is blank for Account ${account.name} ${account.owner} ${account._id}`);
      }    
    } 
    ...
    if (!account.hasOwnProperty('name')) {
      ...
      if (account.name === "" || account.name == undefined || account.name == null) {
        logger.error(`name is blank for Account ${account.name} ${account.owner} ${account._id}`);
      }    
    ...
    if (!account.hasOwnProperty('balance')) {
      ...
      if (isNaN(account.balance)) {
        logger.error(`Account ${account.name} ${account._id} balance is not a number ${account.balance}`);
      }
    }  
    ...
    if (!account.hasOwnProperty('owner')) {
      ...
      if (account.owner === "" || account.owner == undefined || account.owner == null) {
        logger.error(`owner is blank for Account ${account.name} ${account.owner} ${account._id}`);
      }    
    }
    ...
    if (countRef >= 0) {
      ...
      switch(account.code){
        ...
        default:
          logger.error(`Account has invalid code: ${account.code} ${account.name} ${account.owner} ${account._id}`)
      }
    } else {
      logger.error(`Account Failed to Find Team in Count Array: ${teamOwner} ${teamHex} ${account.code} ${account.name} ${account.owner} ${account._id}`);
    }
  }
  ...
  for (tCnt of teamCounts) {
    if (tCnt.acctsCount != 6) {
      logger.error(`Team Owner ${tCnt.owner} ${tCnt.hexId} does not have 6 accounts: : ${tCnt.acctsCount}`);
    } 
    if (tCnt.unsCount != 1) {
      logger.error(`Team Owner ${tCnt.owner} ${tCnt.hexId} does not have 1 UNSC account: ${tCnt.unsCount}`);
    } 
    if (tCnt.treCount != 1) {
      logger.error(`Team Owner ${tCnt.owner} ${tCnt.hexId} does not have 1 Treasury account: ${tCnt.treCount}`);
    } 
    if (tCnt.govCount != 1) {
      logger.error(`Team Owner ${tCnt.owner} ${tCnt.hexId} does not have 1 Governance account: ${tCnt.govCount}`);
    } 
    if (tCnt.opsCount != 1) {
      logger.error(`Team Owner ${tCnt.owner} ${tCnt.hexId} does not have 1 Operations account: ${tCnt.opsCount}`);
    } 
    if (tCnt.sciCount != 1) {
      logger.error(`Team Owner ${tCnt.owner} ${tCnt.hexId} does not have 1 Science account: ${tCnt.sciCount}`);
    } 
    if (tCnt.polCount != 1) {
      logger.error(`Team Owner ${tCnt.owner} ${tCnt.hexId} does not have 1 Political account: ${tCnt.polCount}`);
    }        
  }
