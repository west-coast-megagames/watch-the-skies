...
async function chkResearch(runFlag) {
  ...
  for (const research of await Research.find()
    ...
    if (!research.hasOwnProperty('model')) {
      logger.error(`model missing for Research ${research.name} ${research.code} ${research._id}`);
    }
    ...
    if (!research.hasOwnProperty('type')) {
      ...
      if (!inArray(typeVals, research.type)) {
        logger.error(`Invalid type ${research.type} for Research ${research.name} ${research._id}`);
      }
      ...
      if (research.type === 'Knowledge') {
        if (research.status.completed) {
          if (!research.hasOwnProperty('credit')) {
            ...
            if (!team) {
              logger.error(`team/credit reference is invalid for Knowledge Research ${research.name} ${research._id}`);
            }
          }
        }
        ...
        if (!research.hasOwnProperty('status')) {
          ...
          if (!research.status.hasOwnProperty('available')) {
            logger.error(`status.available missing for Research ${research.name} ${research._id}`);
          }  else {
          ...
          if (!research.status.hasOwnProperty('completed')) {
            logger.error(`status.completed missing for Research ${research.name} ${research._id}`);
          }  else {
          ...
          if (!research.status.hasOwnProperty('published')) {
            logger.error(`status.published missing for Research ${research.name} ${research._id}`);
          }  else {
        ...
        if (!research.hasOwnProperty('teamProgress')) {
          ...
          for (let i = 0; i < research.teamProgress.length; ++i){
            ...
            if (!research.teamProgress[i].hasOwnProperty('progress')) {
              logger.error(`teamProgress.progress missing for Knowledge Research ${i} ${research.name} ${research._id}`);
            }  else {
            ...
            if (!research.teamProgress[i].hasOwnProperty('team')) {
              logger.error(`teamProgress.team missing for Knowledge Research ${i} ${research.name} ${research._id}`);
            }  else {
      ...
      if (research.type === 'Analysis' || research.type === 'Technology') {
        if (!research.hasOwnProperty('team')) {
          ...
          if (!team) {
            logger.error(`team reference is invalid for ${research.type} Research ${research.name} ${research._id}`);
          }
        }
        ...
        if (!research.hasOwnProperty('status')) {
          ...
          if (research.type === 'Technology') {
            if (!research.status.hasOwnProperty('visible')) {
              logger.error(`status.visible missing for Technology Research ${research.name} ${research._id}`);
            }  else {
          ...
          if (!research.hasOwnProperty('salvage')) {
            ...
            for (let i = 0; i < research.salvage.length; ++i){
              ...
              if (!research.salvage[i].hasOwnProperty('outcome')) {
                logger.error(`salvage.outcome missing for Analysis Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.salvage[i].hasOwnProperty('gear')) {
                logger.error(`salvage.gear missing for Analysis Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.salvage[i].hasOwnProperty('system')) {
                logger.error(`salvage.system missing for Analysis Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.salvage[i].hasOwnProperty('facility')) {
                logger.error(`salvage.facility missing for Analysis Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.salvage[i].hasOwnProperty('site')) {
                logger.error(`salvage.site missing for Analysis Research ${i} ${research.name} ${research._id}`);
              }  else {
          ...
          if (!research.hasOwnProperty('theoretical')) {
            ...
            for (let i = 0; i < research.theoretical.length; ++i){
              ...
              if (!research.theoretical[i].hasOwnProperty('name')) {
                logger.error(`theoretical.name missing for Technology Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.theoretical[i].hasOwnProperty('level')) {
                logger.error(`theoretical.level missing for Technology Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.theoretical[i].hasOwnProperty('type')) {
                logger.error(`theoretical.type missing for Technology Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.theoretical[i].hasOwnProperty('code')) {
                logger.error(`theoretical.code missing for Technology Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.theoretical[i].hasOwnProperty('desc')) {
                logger.error(`theoretical.desc missing for Technology Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.theoretical[i].hasOwnProperty('field')) {
                logger.error(`theoretical.field missing for Technology Research ${i} ${research.name} ${research._id}`);
              }  else {
              ...
              if (!research.theoretical[i].hasOwnProperty('prereq')) {
                logger.error(`theoretical.prereq missing for Technology Research ${i} ${research.name} ${research._id}`);
              }  else {
                for (let j = 0; j < research.theoretical[i].prereq[j].length; ++j){
                  if (!research.theoretical[i].prereq[j].hasOwnProperty('code')) {
                    logger.error(`theoretical.prereq.code missing for Technology Research ${i} ${j} ${research.name} ${research._id}`);
                  }  else {
                  ...
                  if (!research.theoretical[i].prereq[j].hasOwnProperty('type')) {
                    logger.error(`theoretical.prereq.type missing for Technology Research ${i} ${j} ${research.name} ${research._id}`);
                  }  else {
    ...
    if (!research.hasOwnProperty('prereq')) {
      ...
      for (let i = 0; i < research.prereq.length; ++i){
        ...
        if (!research.prereq[i].hasOwnProperty('code')) {
          logger.error(`prereq.code missing for Research ${i} ${research.name} ${research._id}`);
        }  else {
        ...
        if (!research.prereq[i].hasOwnProperty('type')) {
          logger.error(`prereq.type missing for Research ${i} ${research.name} ${research._id}`);
        }  else {
    ...
    if (!research.hasOwnProperty('unlocks')) {
      ...
      for (let i = 0; i < research.unlocks.length; ++i){
        if (!research.unlocks[i].hasOwnProperty('code')) {
          logger.error(`unlocks.code missing for Research ${i} ${research.name} ${research._id}`);
        }  else {
        ...
        if (!research.unlocks[i].hasOwnProperty('type')) {
          logger.error(`unlocks.type missing for Research ${i} ${research.name} ${research._id}`);
        }  else {
    ...
    if (!research.hasOwnProperty('breakthrough')) {
      ...
      for (let i = 0; i < research.breakthrough.length; ++i){
        if (!research.breakthrough[i].hasOwnProperty('code')) {
          logger.error(`breakthrough.code missing for Research ${i} ${research.name} ${research._id}`);
        }  else {
        ...
        if (!research.breakthrough[i].hasOwnProperty('type')) {
          logger.error(`breakthrough.type missing for Research ${i} ${research.name} ${research._id}`);
        }  else {
