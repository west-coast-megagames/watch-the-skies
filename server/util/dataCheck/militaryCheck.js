  ...
  for (const military of await Military.find()
    ...
    */
    ...
    if (!military.hasOwnProperty('model')) {
      logger.error(`model missing for Military ${military.name} ${military._id}`);
    }

    if (!military.hasOwnProperty('name')) {
      logger.error(`name missing for Military ${military.name} ${military._id}`);
    }

    if (!military.hasOwnProperty('team')) {
      logger.error(`team missing for Military ${military.name} ${military._id}`);
    } else {
      let team = await Team.findById({'_id': military.team});
      if (!team) {
        logger.error(`team reference is invalid for Military ${military.name} ${military._id}`);
      }
    }

    if (!military.hasOwnProperty('zone')) {
      logger.error(`zone missing for Military ${military.name} ${military._id}`);
    } else {
      let zone = await Zone.findById({'_id': military.zone});
      if (!zone) {
        logger.error(`zone reference is invalid for Military ${military.name} ${military._id}`);
      }
    }

    if (!military.hasOwnProperty('country')) {
      logger.error(`country missing for Military ${military.name} ${military._id}`);
    } else {
      let country = await Country.findById({'_id': military.country});
      if (!country) {
        logger.error(`country reference is invalid for Military ${military.name} ${military._id}`);
      }
    }

    if (!military.hasOwnProperty('site')) {
      logger.error(`site missing for Military ${military.name} ${military._id}`);
    } else {
      let site = await Site.findById({'_id': military.site});
      if (!site) {
        logger.error(`site reference is invalid for Military ${military.name} ${military._id}`);
      }
    }

    if (!military.hasOwnProperty('homeBase')) {
      logger.error(`homeBase missing for Military ${military.name} ${military._id}`);
    } else {
      let site = await Site.findById({'_id': military.homeBase});
      if (!site) {
        logger.error(`homeBase reference is invalid for Military ${military.name} ${military._id}`);
      }
    }

    if (!military.hasOwnProperty('status')) {
      logger.error(`status missing for Military ${military.name} ${military._id}`);
    } else {
      if (!military.status.hasOwnProperty('deployed')) {
        logger.error(`status.deployed missing for Military ${military.name} ${military._id}`);
      }  
      if (!military.status.hasOwnProperty('damaged')) {
        logger.error(`status.damaged missing for Military ${military.name} ${military._id}`);
      }  
      if (!military.status.hasOwnProperty('destroyed')) {
        logger.error(`status.destroyed missing for Military ${military.name} ${military._id}`);
      }  
      if (!military.status.hasOwnProperty('repair')) {
        logger.error(`status.repair missing for Military ${military.name} ${military._id}`);
      }  
      if (!military.status.hasOwnProperty('secret')) {
        logger.error(`status.secret missing for Military ${military.name} ${military._id}`);
      }  
    }

    if (!military.hasOwnProperty('hidden')) {
      logger.error(`hidden missing for Military ${military.name} ${military._id}`);
    }

    if (!military.hasOwnProperty('gear')) {
      logger.error(`gear missing for Military ${military.name} ${military._id}`);
    } else {
    ...
    if (!military.hasOwnProperty('serviceRecord')) {
      logger.error(`serviceRecord missing for Military ${military.name} ${military._id}`);
    } else {
      //militaryCheckDebugger(`Military ${military.name} ${military._id} Check of Gear ${military.gear.length}`);
      for (let i = 0; i < military.serviceRecord.length; ++i){
        ...
        if (!lFind) {
          logger.error(`Military ${military.name} ${military._id} has an invalid serviceRecord reference ${i}: ${military.serviceRecord[i]}`);
        }
      }
    }

    if (!military.hasOwnProperty('type')) {
      logger.error(`type missing for Military ${military.name} ${military._id}`);
    } else {
      if (military.type === "Fleet" || military.type === "Corps") {
        if (!military.hasOwnProperty('stats')) {
          logger.error(`stats missing for Military ${military.name} ${military._id}`);
        } else {
          if (!military.stats.hasOwnProperty('health')) {
            logger.error(`stats.health missing for Military ${military.name} ${military._id}`);
          }  
          if (!military.stats.hasOwnProperty('healthMax')) {
            logger.error(`stats.healthMax missing for Military ${military.name} ${military._id}`);
          }  
          if (!military.stats.hasOwnProperty('attack')) {
            logger.error(`stats.attack missing for Military ${military.name} ${military._id}`);
          }  
          if (!military.stats.hasOwnProperty('defense')) {
            logger.error(`stats.defense missing for Military ${military.name} ${military._id}`);
          }  
          if (!military.stats.hasOwnProperty('localDeploy')) {
            logger.error(`stats.localDeploy missing for Military ${military.name} ${military._id}`);
          }  
          if (!military.stats.hasOwnProperty('globalDeploy')) {
            logger.error(`stats.globalDeploy missing for Military ${military.name} ${military._id}`);
          }  
          if (!military.stats.hasOwnProperty('invasion')) {
            logger.error(`stats.invasion missing for Military ${military.name} ${military._id}`);
          }  
        }
      } else {
        logger.error(`Invalid Type ${military.type} for Military ${military.name} ${military._id}`);
      }
    }
    ...
    try {
      ...
      if ( error)  {
        logger.error(`Military Validation Error For ${military.name} ${military._id} Error: ${error.details[0].message}`);
      }
    } catch (err) {
      logger.error(`Military Validation Error For ${military.name} ${military._id} Error: ${err.details[0].message}`);
    }  
