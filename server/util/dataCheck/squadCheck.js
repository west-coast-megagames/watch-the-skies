...
async function chkSquad(runFlag) {
  ...
  for (const squad of await Squad.find()
    ...
    if (!squad.hasOwnProperty('zone')) {
      ...
      if (!zone) {
        logger.error(`zone reference is invalid for Squad ${squad.name} ${squad._id}`);
      }
    }
    ...
    if (!squad.hasOwnProperty('team')) {
      ...
      if (!team) {
        logger.error(`team reference is invalid for Squad ${squad.name} ${squad._id}`);
      }
    }
    ...
    if (!squad.hasOwnProperty('country')) {
      ...
      if (!country) {
        logger.error(`country reference is invalid for Squad ${squad.name} ${squad._id}`);
      }
    }
    ...
    if (!squad.hasOwnProperty('homeBase')) {
      ...
      if (!site) {
        logger.error(`homeBase reference is invalid for Squad ${squad.name} ${squad._id}`);
      }
    }
    ...
    if (!squad.hasOwnProperty('site')) {
      ...
      if (!site) {
        logger.error(`site reference is invalid for Squad ${squad.name} ${squad._id}`);
      }
    }
    ...
    if (!squad.hasOwnProperty('status')) {
      ...
      if (!squad.status.hasOwnProperty('damaged')) {
        logger.error(`status.damaged missing for Squad ${squad.name} ${squad._id}`);
      }  else {
        if (squad.status.damaged === undefined || squad.status.damaged === null) {
          logger.error(`Squad status.damaged is not set ${squad.name} ${squad._id}`);
        }   
      }
      
      if (!squad.status.hasOwnProperty('deployed')) {
        logger.error(`status.deployed missing for Squad ${squad.name} ${squad._id}`);
      }  else {
        if (squad.status.deployed === undefined || squad.status.deployed === null) {
          logger.error(`Squad status.deployed is not set ${squad.name} ${squad._id}`);
        }                    
      }

      if (!squad.status.hasOwnProperty('destroyed')) {
        logger.error(`status.destroyed missing for Squad ${squad.name} ${squad._id}`);
      }  else {
        if (squad.status.destroyed === undefined || squad.status.destroyed === null) {
          logger.error(`Squad status.destroyed is not set ${squad.name} ${squad._id}`);
        }     
      }

      if (!squad.status.hasOwnProperty('repair')) {
        logger.error(`status.repair missing for Squad ${squad.name} ${squad._id}`);
      }  else {
        if (squad.status.repair === undefined || squad.status.repair === null) {
          logger.error(`Squad status.repair is not set ${squad.name} ${squad._id}`);
        }     
      }

      if (!squad.status.hasOwnProperty('secret')) {
        logger.error(`status.secret missing for Squad ${squad.name} ${squad._id}`);
      }  else {
        if (squad.status.repair === undefined || squad.status.repair === null) {
          logger.error(`Squad status.secret is not set ${squad.name} ${squad._id}`);
        }     
      }
    }
    ...
    if (!squad.hasOwnProperty('type')) {
      ...
      if (!inArray(typeVals, squad.type)) {
        logger.error(`Invalid type ${squad.type} for Squad ${squad.name} ${squad._id}`);
      }
    }
    ...
    try {
      ...
      logger.error(`Squad Validation Error For ${squad.name} Error: ${err.details[0].message}`);
    }
