...
router.post("/build", async function (req, res) {
  ...
  if (!docs.length) {
          ...
          await newSystem.save((err, newSystem) => {
            if (err)
              return console.error(
                `Post Build Aircraft System Save Error: ${err}`
              );
          });
    ...
    if (aircraft.code && aircraft.code != "") {
      let zone = await Zone.findOne({ code: aircraft.code });
      if (!zone) {
        console.log(
          "Aircraft Build Zone Error, New Aircraft:",
          req.body.name,
          " Zone: ",
          req.body.code
        );
    ...
    if (aircraft.teamCode && aircraft.teamCode != "") {
      ...
      if (!team) {
        console.log(
          "Aircraft Build Team Error, New Aircraft:",
          req.body.name,
          " Team: ",
          req.body.teamCode
        );
    ...
    if (aircraft.countryCode && aircraft.countryCode != "") {
      ...
      if (!country) {
        console.log(
          "Aircraft Build Country Error, New Aircraft:",
          req.body.name,
          " Country: ",
    ...
    if (
      aircraft.baseCode &&
      aircraft.baseCode != "" &&
      aircraft.baseCode != "undefined"
    ) {
      let baseSite = await BaseSite.findOne({ siteCode: aircraft.baseCode });
      if (!baseSite) {
        console.log(
          "Aircraft Build Base Error, New Aircraft:",
          req.body.name,
          " Base: ",
          req.body.baseCode
        );
    ...
    if (error) {
      ...
      return res
        .status(400)
        .send(
          `Aircraft Validate Error ${aircraft.name} Error: ${error.message}`
        );
    }
    ...
    console.log(
      `Aircraft already exists (control post build): ${aircraft.name}`
    );
