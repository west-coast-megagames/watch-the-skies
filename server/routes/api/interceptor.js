...
router.post("/", async function (req, res) {
  ...
  if (!docs.length) {
    if (zoneCode && zoneCode != "") {
      ...
      if (!zone) {
        console.log(
          "Aircraft Post Zone Error, New Aircraft:",
          req.body.name,
          " Zone: ",
          req.body.zoneCode
        );
    ...
    if (teamCode && teamCode != "") {
      ...
      if (!team) {
        console.log(
          "Aircraft Post Team Error, New Aircraft:",
          req.body.name,
          " Team: ",
          req.body.teamCode
        );
    ...
    if (countryCode && countryCode != "") {
      ...
      if (!country) {
        console.log(
          "Aircraft Post Country Error, New Aircraft:",
          req.body.name,
          " Country: ",
    ...
    if (baseCode && baseCode != "" && baseCode != "undefined") {
      ...
      if (!baseSite) {
        console.log(
          "Aircraft Post Base Error, New Aircraft:",
          req.body.name,
          " Base: ",
          baseCode
        );
          ...
          if (validUnitType(sysRef.unitType, NewAircraft.type)) {
            ...
            await newSystem.save((err, newSystem) => {
              if (err) {
                ...
                res
                  .status(400)
                  .send(`Aircraft System Save Error ${name} Error: ${err}`);
              }
            });
            newAircraft.systems.push(newSystem._id);
          } else {
            console.log(
              "Error in creation of system - wrong UnitType",
              sys,
              " for ",
              name
            );
    ...
    if (error) {
      ...
      for (let j = 0; j < newAircraft.systems.length; ++j) {
        ...
        if ((systemDel = null)) {
          console.log(
            `The Aircraft System with the ID ${sysId} was not found!`
          );
        }
      }
      res
        .status(400)
        .send(`Aircraft Validate Error ${name} Error: ${error.message}`);
    }
...
router.put("/:id", async function (req, res) {
  ...
  if (zoneCode && zoneCode != "") {
    ...
    if (!zone) {
      console.log(
        "Aircraft Put Zone Error, Update Aircraft:",
        req.body.name,
        " Zone: ",
        zoneCode
      );
  ...
  if (teamCode && teamCode != "") {
    ...
    if (!team) {
      console.log(
        "Aircraft Put Team Error, Update Aircraft:",
        req.body.name,
        " Team: ",
        teamCode
      );
  ...
  if (countryCode && countryCode != "") {
    ...
    if (!country) {
      console.log(
        "Aircraft Put Country Error, Update Aircraft:",
        req.body.name,
        " Country: ",
        countryCode
      );
  ...
  if (baseCode && baseCode != "" && baseCode != "undefined") {
    ...
    if (!baseSite) {
      console.log(
        "Aircraft Put Base Error, Update Aircraft:",
        req.body.name,
        " Base: ",
        baseCode
      );
  ...
  let aircraft = await Aircraft.findOneAndUpdate(
    { _id: req.params.id },
    {
      name,
      zone: newZone_Id,
