...
router.post('/', async function (req, res) {
  ...
  if (!docs.length) {
    ...
    if (zoneCode && zoneCode != ""){
      ...
      if (!zone) {
        console.log("Military Post Zone Error, New Military:", req.body.name, " Zone: ", req.body.zoneCode);
      } else {
    ...
    if (teamCode && teamCode != ""){
      ...
      if (!team) {
        console.log("Military Post Team Error, New Military:", req.body.name, " Team: ", req.body.teamCode);
      } else {
    ...
    if (countryCode && countryCode != ""){
      ...
      if (!country) {
        console.log("Military Post Country Error, New Military:", req.body.name, " Country: ", req.body.countryCode);
      } else {
        ...
        if (gerRef) {
          ...
          await newGear.save(((err, newGear) => {
            if (err) {
              ...
              res.status(400).send(`Military Gear Save Error ${name} Error: ${err}`);   
            }
          }));
    ...
    if (error) {
      console.log("New Military Validate Error", newMilitary.name, error.message);
      ...
      res.status(400).send(`Military Validate Error ${name} Error: ${error.message}`);   
    }
...
router.put('/:id', async function (req, res) {
  ...
  if (zoneCode && zoneCode != "") {
    let zone = await Zone.findOne({ zoneCode: zoneCode });  
    if (!zone) {
      console.log("Military Put Zone Error, Update Military:", req.body.name, " Zone: ", zoneCode);
    } else {
      newZone_Id  = zone._id;
    }
  } else {
  ...
  if (teamCode && teamCode != "") {
    let team = await Team.findOne({ teamCode: teamCode });  
    if (!team) {
      console.log("Military Put Team Error, Update Military:", req.body.name, " Team: ", teamCode);
    } else {
      newTeam_Id  = team._id;
    }
  } else {
  ...
  if (countryCode && countryCode != "") {
    let country = await Country.findOne({ code: countryCode });  
    if (!country) {
      console.log("Military Put Country Error, Update Military:", req.body.name, " Country: ", countryCode);
    } else {
  ...
  let military = await Military.findOneAndUpdate({ _id: req.params.id }, 
    { name,
      zone: newZone_Id,
